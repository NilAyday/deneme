...
plt.hist(np.array(s_30['loss_clean']),range=(1,5),bins=60,alpha=0.5,label="clean data")
plt.hist(np.array(s_30['loss']),range=(1,5),bins=60,alpha=0.5,label="corrupted data 0.3")
plt.legend()
#plt.show()
file= os.path.join(os.path.join(os.path.dirname(__file__)), 'fig4a')
plt.savefig(file+".png")
plt.clf()
plt.hist(np.array(s_50['loss_clean']),range=(6,10),bins=60,alpha=0.5,label="clean data")
plt.hist(np.array(s_50['loss']),range=(6,10),bins=60,alpha=0.5,label="corrupted data 0.5")
plt.legend()
#plt.show()
